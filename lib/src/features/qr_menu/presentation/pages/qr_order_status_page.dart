...
class QROrderStatusPage extends StatefulWidget {
  final String orderId;
  final Restaurant restaurant;
  const QROrderStatusPage({
    super.key,
  ...
  @override
  State<QROrderStatusPage> createState() => _QROrderStatusPageState();
}

class _QROrderStatusPageState extends State<QROrderStatusPage> {
  @override
  void initState() {
    super.initState();

    context.read<QROrderStatusBloc>().add(
          GetQROrderStatusEvent(
            orderId: widget.orderId,
          ),
        );
    QROrderSocketMethod().getOrderStatus(context, widget.restaurant.id);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: BlocConsumer<QROrderSocketStatusBloc, QROrderSocketStatusState>(
            listener: (context, socketState) {
          if (socketState is QROrderSocketConnectedState) {
            QROrderSocketMethod().getOrderStatus(context, widget.restaurant.id);
            context.read<QROrderStatusBloc>().add(
                  GetQROrderStatusEvent(
                    orderId: widget.orderId,
          ...
          if (socketState is QROrderSocketLoadingState) {
            return SizedBox(
              height: 100.h,
              width: 100.w,
              child: Center(
          ...
          } else if (socketState is QROrderSocketFailedState) {
            return SizedBox(
              height: 100.h,
              width: 100.w,
              child: Center(
                child: ErrorAndInfoDisplayWidget(
                  ...
                  onPressed: () {
                    dpLocator<QROrderSocketStatusBloc>().add(
                      QROrderConnectSocket(),
                    );
                  },
                ),
              ),
          ...
          return BlocConsumer<QROrderStatusBloc, QROrderStatusState>(
            listener: (_, state) {},
            builder: (context, orderState) {
              if (orderState is QROrderStatusInitial ||
                  orderState is QROrderStatusUpdatedInProgress) {
                return SizedBox(
                  height: 100.h,
                  width: 100.w,
                  child: Center(
              ...
              } else if (orderState is QROrderStatusUpdateFailed) {
                return SizedBox(
                  height: 100.h,
                  width: 100.w,
                  child: Center(
              ...
              } else if (orderState is QROrderStatusUpdated) {
                ...
                return Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      GestureDetector(
                        onTap: () {
                          context.pushNamed(AppRoutes.editQROrderPage,
                              pathParameters: {
                                "orderId": widget.orderId,
              ...
              } else if (orderState is QROrderRejected) {
                return Center(
                  child: Text(
                    "Order rejected",
      ...
      bottomNavigationBar: BlocBuilder<QROrderStatusBloc, QROrderStatusState>(
        builder: (context, state) {
          if (state is QROrderStatusUpdated) {
                    ...
                    child: Container(
                      ...
                      child: OrderButton(
                        onTap: () {
                          context
                              .read<GetQROrderBloc>()
                              .add(GetQROrder(orderId: widget.orderId));
                          showModalBottomSheet(
                            context: context,
                            builder: (BuildContext context) {
                              return BlocBuilder<GetQROrderBloc,
                                  GetQROrderState>(builder: (context, state) {
                                if (state is GetQROrderLoading) {
                                  return SizedBox(
                                    height: 40.h,
                                    child: Center(
                                      child:
                                ...
                                } else if (state is GetQROrderSuccess) {
                                  return Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Row(
                                          ...
                                          children: [
                                            Text(
                                                'Order ${state.qrOrder.orderNumber}'),
                                            Text(
                                              "Bill requested",
                                              style: TextStyle(),
                                            )
                                          ...
                                          child: SingleChildScrollView(
                                            child: Column(
                                                ...
                                                children: List.generate(
                                                  state.qrOrder.orderItems
                                                      .length,
                                                  (index) {
                                                    final qrItem = state
                                                        .qrOrder.orderItems.keys
                                                        .toList()[index];
                                                    return IntrinsicHeight(
                                                      child: Column(
                                                        children: [
                                                          SizedBox(height: 8),
                                                          Text(
                                                              '${qrItem.name}\nQty: ${state.qrOrder.orderItems[qrItem]}\nPrice: ${qrItem.price} ETB'),
                                                          SizedBox(height: 16),
                                                        ],
                                                      ),
                                                    );
                                        ...
                                        SizedBox(height: 16),
                                        Text(
                                          'Total Amount: ${state.qrOrder.totalPrice}',
                                          style: TextStyle(
                                            fontSize: 18,
                                            fontWeight: FontWeight.bold,
                                          ),
                                  ...
                                  return ErrorAndInfoDisplayWidget(
                                    assetImage: "assets/icons/no_internet.svg",
                                    title: "Failed to Get Bill",
                                    description: state is GetQROrderFailure
                                        ? state.message
                                        : "Something went wrong please retry",
                                    onPressed: () {
                                      context.read<GetQROrderBloc>().add(
                                          GetQROrder(orderId: widget.orderId));
                                    },
                                  );
                                }
                              });
  ...
  List<QROrderStep> getOrderStatusModels(QROrderStatusUpdated state) {
    final statusTitles = [
  ...
  QROrderProgressModel getOrderProgressModel(QROrderStatusUpdated state) {
    final orderStatusList = [
      state.created,
      state.confirmed,
      state.paid,
...
class QROrderStep extends StatelessWidget {
  final String time;
  final String title;
  final String orderId;
...
class QROrderProgressModel {
  final String text;
  final String subtext;
  final Color backgroundColor;
  final Color subtextColor;

  QROrderProgressModel({
    required this.text,
