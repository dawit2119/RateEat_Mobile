...
class AddFoodReviewDp {
  final Dio dio;

  AddFoodReviewDp({required this.dio});

  Future<void> addFoodReview({
    ...
    try {
      ...
      Future<void> addMediaToFile(File file, String type) async {
        ...
        if (['jpg', 'jpeg', 'png'].contains(fileExt)) {
          type = "item_review_images";
        } else if (['mp4', 'mov'].contains(fileExt)) {
          type = "item_review_videos";
        }
        formData.files.add(
          MapEntry(
            type,
