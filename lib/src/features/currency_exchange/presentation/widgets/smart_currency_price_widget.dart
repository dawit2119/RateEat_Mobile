...
/// 1. Data Source: Full list of currencies
class CurrencyData {
...
/// 2. Main Widget: The Price Display
class SmartCurrencyPriceWidget extends StatefulWidget {
  final double originalPrice;
  final String originalCurrency;
  final TextStyle? priceStyle;

  const SmartCurrencyPriceWidget({
    Key? key,
  ...
  @override
  State<SmartCurrencyPriceWidget> createState() =>
      _SmartCurrencyPriceWidgetState();
}

class _SmartCurrencyPriceWidgetState extends State<SmartCurrencyPriceWidget> {
  late String currentCurrency;
  late double currentPrice;
  bool isConverting = false;
  double? savedExchangeRate;
  StreamSubscription<CurrencyState>? _currencySubscription;
  ...
  void _convertCurrency(String toCurrency) {
    ...
    _currencySubscription?.cancel();
    _currencySubscription = currencyBloc.stream.listen((state) {
      if (state is CurrencySuccess) {
        if (!mounted) return;
        setState(() {
          savedExchangeRate =
              state.conversion.convertedAmount / widget.originalPrice;
    ...
    currencyBloc.add(ConvertCurrencyEvent(
  ...
  Widget build(BuildContext context) {
    // FIX: Wrapped in FittedBox to scale down content if space is tight
    return GestureDetector(
      onTap: isConverting ? null : _showCurrencySelector,
      child: FittedBox(
        fit: BoxFit.scaleDown,
        alignment: Alignment.centerLeft,
        child: Container(
          // Minimized padding to save space
...
/// 3. Helper Widget: The Searchable Bottom Sheet
        ...
        children: [
          ...
          Expanded(
            child: filteredCurrencies.isEmpty
                ? Center(child: Text("No currency found"))
                : ListView.builder(
